...
impl FanCurveClient {
    ...
    pub async fn new() -> Result<Self> {
        let connection = Connection::system().await
            .map_err(|e| FanCurveError::DBus(e))?;
    ...
    pub async fn handle_args(&self, args: Args) -> Result<()> {
        match args.command {
            Some(Commands::Daemon) => {
                ...
                Err(FanCurveError::Unknown("Invalid command for client".to_string()))
            }
            Some(Commands::FanCurve { command }) => {
                self.handle_fan_curve_command(command).await
            }
            None => {
    ...
    async fn handle_fan_curve_command(&self, command: FanCurveCommands) -> Result<()> {
        match command {
            FanCurveCommands::List => {
                self.list_fan_curves().await
            }
            FanCurveCommands::Get => {
                self.get_current_fan_curve().await
            }
            FanCurveCommands::Set { name } => {
                self.set_fan_curve_by_name(&name).await
            }
            FanCurveCommands::SetDefault { name } => {
                self.set_default_fan_curve(&name).await
            }
            FanCurveCommands::AddPoint { temp, duty } => {
                self.add_fan_curve_point(temp, duty).await
            }
            FanCurveCommands::RemovePoint => {
                self.remove_fan_curve_point().await
            }
            FanCurveCommands::Save => {
                self.save_config().await
            }
            FanCurveCommands::Load => {
    ...
    async fn test_fan_curve(&self, duration: u64, log_file: Option<&str>) -> Result<()> {
        ...
        let log_path = log_file.map(|p| std::path::Path::new(p));
