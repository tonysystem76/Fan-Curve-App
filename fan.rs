...
impl FanDaemon {
    ...
    pub fn set_fan_curve(&mut self, name: Option<String>, curve: Vec<(u8, u8)>, set_as_default: bool) -> Result<(), String> {
        ...
        for (temp, duty) in curve {
            ...
            let temp = i16::from(temp) * 100;
            // Convert duty from percentage to the internal representation (hundredths of a percent)
            let duty = u16::from(duty) * 100;
            
            new_curve.add_point(temp, duty);
        }

        // Validate the new curve
        if new_curve.is_empty() {
            return Err("Fan curve must contain at least one point".to_string());
        }

        // Check if points are in ascending order
        if new_curve.points().windows(2).any(|w| w[0].temp >= w[1].temp) {
            return Err("Fan curve points must be in ascending order by temperature".to_string());
        }

        // Replace the existing curve with the new one
        self.curve = new_curve;
        
        // Immediately apply the new curve
        self.step();
    ...
    pub fn save_fan_curve(&self, path: &Path, name: String) -> std::io::Result<()> {
        ...
        let dir = path.parent().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "Invalid path"))?;
        let mut temp_file = NamedTempFile::new_in(dir)?;
        
        //Write the new content to the temporary file
        temp_file.as_file_mut().write_all(json.as_bytes())?;
        temp_file.as_file_mut().flush()?;
        
        // Attempt to create a backup of the existing file
        if path.exists() {
            let backup_path = path.with_extension("json.bak");
            fs::copy(path, &backup_path).ok(); //Ignore errots, it's just a backup
        }
        
        //Atomically rename the temporary file to the target path
        temp_file.persist(path)?;
        
        info!("Successfully wrote the fan curve to {:?}", path);
        Ok(())
    }
    ...
    pub fn get_fan_curve(&self) -> Vec<(u8, u8)> {
        self.curve.points()
            .iter()
...
impl FanCurve {
    pub fn new(name: String) -> Self {
     ...
     pub fn last_point(&self) -> Option<&FanPoint> {
         self.points.last()
     }
    ...
    pub fn threadripper2() -> Self {
        Self::new("Threadripper 2".to_string())
            .append(00_00, 30_00)
            .append(40_00, 40_00)
            .append(47_50, 50_00)
            .append(55_00, 65_00)
...
mod tests {
    ...
    fn threadripper2_points() {
        ...
        assert_eq!(threadripper2.name(), "Threadripper 2");
    ...
    #[test]
    fn custom_curve() {
        let mut custom = FanCurve::new("Custom".to_string());
        custom.add_point(0, 20);
        custom.add_point(50, 50);
        custom.add_point(100, 100);
    
        assert_eq!(custom.name(), "Custom");
        assert_eq!(custom.get_duty(0), Some(2000));
        assert_eq!(custom.get_duty(25), Some(3500));
        assert_eq!(custom.get_duty(50), Some(5000));
        assert_eq!(custom.get_duty(75), Some(7500));
        assert_eq!(custom.get_duty(100), Some(10000));
    }
}
