...
#[derive(Parser)]
#[clap(
    about = "Query or set the fan curve",
    long_about = "Query or set the fan curve.\n\n - If no arguments are provided, the current fan curve will be displayed\n - Otherwise, the fan curve will be set according to the provided points"
)]
pub enum FanCurveArgs {
    #[clap(about = "Display the current fan curve")]
    Get,
    #[clap(about = "Set a new fan curve")]
    Set {
        #[clap(
            help = "Fan curve points as pairs of temperature (°C) and fan speed (%)",
            value_parser = clap::value_parser!(u8).range(0..=100),
            num_args = 2..,
            value_names = &["temp", "speed"],
        )]
        points: Vec<u8>,
    },
}
...
#[clap(
    name = "system76-power",
    about = "Utility for managing graphics, power profiles, and fan curves",
...
pub enum Args {
    ...
    #[clap(about = "Query or set the fan curve")]
    FanCurve {
        #[clap(subcommand)]
        cmd: Option<FanCurveArgs>,
    },
    #[clap(about = "Run the fan curve control GUI")]
    Gui,
}
